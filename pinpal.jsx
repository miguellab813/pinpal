...
const Calendar = ({vials,entries}) => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {/* ── Active levels section ── */}
      {(()=>{
        const now = new Date();
        // For each unique peptide in ALL entries, compute current active amount
        const activeMap = {};
        entries.forEach(e => {
          const name = e.vial_name;
          const hl_data = Object.entries(HALF_LIVES).find(([k]) =>
            name.toLowerCase().includes(k.toLowerCase())
          );
          const hl = hl_data ? hl_data[1].hl : null;
          const hlType = hl_data ? hl_data[1].type : "medium";
          if(!e.dose_mcg || !hl) return;
          const hoursSince = (now - new Date(e.injected_at)) / 3600000;
        ...
        return (
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <div style={{fontSize:11,fontWeight:700,color:T.textSub,textTransform:"uppercase",letterSpacing:.7}}>Currently Active</div>
              <span style={{fontSize:10,color:T.textMute}}>based on half-life</span>
            </div>
            {activeList.map(a=>{
              const isShort = a.hlType==="short";
              const displayAmt = a.active>=1000
              ...
              const hlLabel = a.hl>=24
                ...
                :a.hl<1
                  ?Math.round(a.hl*60)+"min half-life"
                  :a.hl+"h half-life";
              return (
                <Card key={a.name} style={{padding:"10px 14px",
                  borderLeft:`3px solid ${isShort?"#555":(a.dot||T.accent)}`,
                  opacity:isShort?.75:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    <span style={{width:8,height:8,borderRadius:"50%",background:isShort?"#666":(a.dot||T.accent)}}/>
                    <span style={{fontSize:14,fontWeight:800,color:isShort?T.textSub:T.text,flex:1}}>{a.name}</span>
                    {isShort && (
                      <span style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase",letterSpacing:.5,padding:"1px 5px",borderRadius:4,background:"rgba(255,255,255,.06)",border:"1px solid #333"}}>Short HL</span>
                    )}
                    <span style={{fontSize:11,color:T.textMute}}>{hlLabel}</span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{flex:1}}>
                      <div style={{height:4,background:T.elevated,borderRadius:99}}>
                        <div style={{height:4,borderRadius:99,width:`${pctActive}%`,background:barColor,transition:"width .4s"}}/>
                      </div>
                    </div>
                    <span style={{fontSize:13,fontWeight:700,color:barColor,flexShrink:0}}>{displayAmt}</span>
                  </div>
                  <div style={{fontSize:10,color:T.textMute,marginTop:4}}>
                    {pctActive.toFixed(0)}% of last dose still active
                  </div>
                </Card>
              );
            })}
